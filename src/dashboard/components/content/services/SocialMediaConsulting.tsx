...
import "../../../../index.css";
import SMConsultingReply from "../../modal_content/services/sm-consulting/SMConsultingReply";
import SMConsultingView from "../../modal_content/services/sm-consulting/SMConsultingView";
import type { SMConsulting } from "../../../types/services/services";
...
const columns = [
  ...
  { id: "platforms", label: "Platforms", minWidth: 100 },
  { id: "goals", label: "Goals", minWidth: 120 },
...
export default function SMConsulting({}: SMConsultingProps) {
  const [rows, setRows] = useState<SMConsulting[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<SMConsulting | null>(null);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get<SMConsulting[]>(
        ApiRoutes.Services.getSMCServiceData
      );
      setRows(res.data);
    } catch (err) {
    ...
    <>
      ...
      <CustomModal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
        ...
        {modalMode === "view" && selectedItem && (
          <SMConsultingView setIsOpen={setModalOpen} item={selectedItem} />
        )}
      </CustomModal>
    </>
  );
