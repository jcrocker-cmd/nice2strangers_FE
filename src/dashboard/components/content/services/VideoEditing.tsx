...
import "../../../../index.css";
import VEReply from "../../modal_content/services/video-editing/VEReply";
import VEView from "../../modal_content/services/video-editing/VEView";
...
const columns = [
  ...
  { id: "otherContentType", label: "Other Content Type", minWidth: 100 },
...
export default function VideoEditing({}: SMCreationProps) {
  const [rows, setRows] = useState<VideoEditing[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<VideoEditing | null>(null);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get<VideoEditing[]>(
        ApiRoutes.Services.getVEServiceData
      );
      setRows(res.data);
    } catch (err) {
    ...
    <>
      ...
      <CustomModal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
        ...
        {modalMode === "view" && selectedItem && (
          <VEView setIsOpen={setModalOpen} item={selectedItem} />
        )}
      </CustomModal>
    </>
  );
