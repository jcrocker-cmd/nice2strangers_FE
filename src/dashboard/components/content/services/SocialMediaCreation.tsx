...
import "../../../../index.css";
import SMCreationReply from "../../modal_content/services/sm-creation/SMCreationReply";
import SMCreationView from "../../modal_content/services/sm-creation/SMCreationView";
import type { SMCreation } from "../../../types/services/services";
...
const columns = [
  ...
  { id: "frequency", label: "Posting Frequency", minWidth: 120 },
...
export default function SMCreation({}: SMCreationProps) {
  const [rows, setRows] = useState<SMCreation[]>([]);
  ...
  const [selectedItem, setSelectedItem] = useState<SMCreation | null>(null);
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await axios.get<SMCreation[]>(
        ApiRoutes.Services.getSMCCServiceData
      );
      setRows(res.data);
    } catch (err) {
    ...
    <>
      ...
      <CustomModal isOpen={modalOpen} onClose={() => setModalOpen(false)}>
        ...
        {modalMode === "view" && selectedItem && (
          <SMCreationView setIsOpen={setModalOpen} item={selectedItem} />
        )}
      </CustomModal>
    </>
  );
